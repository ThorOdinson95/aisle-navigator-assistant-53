...
import { Sparkles, Lightbulb, BadgePercent } from "lucide-react";
...
const PersonalizedSuggestions = ({ items }: PersonalizedSuggestionsProps) => {
  const { relevantDeals, relevantAlternatives, storeWideDeals } = useMemo(() => {
    const storeDeals = allDeals.filter(deal => deal.type === 'deal' && deal.isStoreWide);
  ...
  const hasStoreDeals = storeWideDeals.length > 0;

  if (!hasPersonalizedContent && !hasStoreDeals) {
    return null;
  }
    ...
    <Card className="transition-transform duration-200 hover:scale-105">
      ...
      <CardContent>
        <ScrollArea className="h-[450px] w-full">
            {items.length > 0 && !hasPersonalizedContent && (
            ...
            {hasPersonalizedContent && hasStoreDeals && <hr className="my-4" />}

            {hasStoreDeals && (
                <div className={!hasPersonalizedContent && items.length === 0 ? "" : "mt-4"}>
                    <h4 className="font-semibold mb-2 text-sm text-muted-foreground flex items-center gap-2"><BadgePercent className="h-4 w-4 text-primary"/>Store-wide Deals</h4>
                    <ul className="space-y-2 pr-4">
